// Parser Financeiro Resiliente - NUNCA FALHA
class FinanceParser {
  constructor() {
    this.categorias = {
  ...
  analisarSegmento(segmento, warnings) {
    ...
    return { type: tipo, amount: valor, currency: 'BRL', date: data, description: descricao, category: categoria };
  }
  ...
  detectarCategoria(segmento, tipo, descricao) {
    ...
    for (const [cat, palavras] of Object.entries(this.categorias)) {
      ...
      if ((cat === 'salário' || cat === 'freelance') && tipo !== 'entrada') continue;
