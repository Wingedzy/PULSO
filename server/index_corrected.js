...
app.post('/api/financas', (req, res) => {
  ...
  if (mensagem_nl) {
    ...
    if (sugestao.valido) {
      transacao = sugestao.transacoes[0];
      // Permitir override com campos manuais
      transacao = {
        ...transacao,
        tipo: tipo || transacao.type,
        valor: valor || transacao.amount,
        data: data || transacao.date,
        descricao: descricao || transacao.description,
        categoria: categoria || transacao.category
      };
    } else {
